...
export const Demo: Story = {
  render: (args, context) => {
    ...
    const [formats, setFormats] = useState(() => [value[1], value[2], value[6]]);
    ...
    return (
      ...
      >
        <ToggleButton aria-label={value[0]} value={value[0]}>
          {getText(args, context)} 1
        </ToggleButton>
        <ToggleButton aria-label={value[1]} value={value[1]}>
          {getText(args, context)} 2
        </ToggleButton>
        <ToggleButton aria-label={value[2]} value={value[2]}>
          {getText(args, context)} 3
        </ToggleButton>
        <ToggleButton aria-label={value[3]} value={value[3]}>
          {getText(args, context)} 4
        </ToggleButton>
        <ToggleButton aria-label={value[4]} value={value[4]}>
          {getText(args, context)} 5
        </ToggleButton>
        <ToggleButton aria-label={value[5]} disabled={true} value={value[5]}>
          {getText(args, context)} 6
        </ToggleButton>
        <ToggleButton aria-label={value[6]} disabled={true} value={value[6]}>
