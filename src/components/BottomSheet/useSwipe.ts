...
function getHandlers(
  ...
  const onMove = (event: HandledEvents) => {
    set((state, props) => {
      ...
      if (props.onSwiping || props.onSwiped || props[`onSwiped${dir}` as keyof SwipeableDirectionCallbacks]) {
    ...
    set((state, props) => {
      ...
      if (state.swiping && state.eventData) {
        ...
        if (event.timeStamp - state.start < props.swipeDuration) {
          ...
          const onSwipedDir = props[`onSwiped${eventData.dir}` as keyof SwipeableDirectionCallbacks];
...
export function useSwipeable(options: SwipeableProps): SwipeableHandlers {
  ...
  const [handlers, attachTouch] = React.useMemo(
    () =>
      getHandlers(
        (stateSetter) => (transientState.current = stateSetter(transientState.current, transientProps.current)),
        { trackMouse }
      ),
    [trackMouse]
  );
